...
error = exact_solution.compute_error_of_exact_solution(t, y, y_exact, error_type = 'absolute')

axes[0][0].text(t0 + 0.1*(tf-t0), 0.9*max(y[:,0]), r'$\langle \mathcal{E}_{\mathrm{abs}} \rangle = %.2e$' % error, fontsize=10)
...
fig.tight_layout()
# fig.savefig("test_implicit.png", dpi = 200)
plt.show()
