...
def plot_test(y, t, dt, t0, tf, method, adaptive, rejection_rate, function_evaluations, plot_variables, error, log_plot = False):
	...
	axes[0][0].text(t0 + 0.1*(tf-t0), 0.3*abs(np.amax(y)) + np.amax(y), r'$\mathcal{E}^{(\mathrm{max})}_{\mathrm{abs}} = %.2e$' % error, fontsize=10)
	...
	axes[0][1].set_ylim(0, 1.25*max(dt))
	# axes[0][1].axhline(0, color = 'black', linewidth = 0.3)
	# axes[0][1].axhline(max(dt), color = 'black', linewidth = 0.3)
